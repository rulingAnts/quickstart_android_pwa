...
## Prompt B: Regenerate with Improvements

Use this prompt to build an enhanced version incorporating your later feature specifications:

---

**Build an enhanced "Wordlist Elicitation Tool" PWA with the following additions to the base structure in Prompt A:**

### Enhanced Features

#### 1. Strict Audio Capability Detection

**Requirement**: Detect 16-bit WAV support at startup; block elicitation if unsupported.

**Implementation** (`js/audio-recorder.js`):
- Add `async checkWavSupport()` method:
  - Test cycle: create silent MediaRecorder → record short clip → decode → convert to 16-bit PCM → validate header
  - Return `{ supported: boolean, message: string }`
- Call from `app.init()` before enabling elicitation
- If unsupported: show persistent banner with message "Your browser does not support 16-bit WAV recording. Please use Chrome, Edge, or Firefox." and disable Record button

**Success criteria**: Firefox/Chrome/Edge pass; Safari may fail (show blocking UI)

#### 2. UTF-16 Import/Export

**Import** (`js/xml-parser.js`):
- Add `async parseWordlistFromSource(source, isUrl)`:
  - If `isUrl`: fetch with proper CORS handling
  - Detect encoding via BOM or heuristic (UTF-16LE: FF FE, UTF-16BE: FE FF, else UTF-8)
  - Use `TextDecoder` with detected encoding
  - Parse normalized XML

**Export** (`js/export.js`):
- Modify `generateXML()`:
  - Use `TextEncoder` with UTF-16LE
  - Prepend BOM: `0xFF, 0xFE`
  - Schema: `<phon_data>` root, `<data_form>` entries
  - Fields: `<Reference>`, `<Gloss>`, `<LocalTranscription>`, `<SoundFile>`, `<Picture>`, `<RecordedAt>`

#### 3. Reference Sorting and Schema Enforcement

**Import normalization** (`js/xml-parser.js`):
- Parse `reference` as integer (strip non-digits if needed)
- Store as padded string: `String(numericRef).padStart(4, '0')`
- After parsing all entries, sort by numeric reference ascending
- Enforce audio filename rule via `generateAudioFilename(reference, gloss)`:
  - Slug: lowercase, spaces → dots, strip/replace non `[a-z0-9._-]`, max 64 chars
  - Format: `${reference}_${slug}.wav`

#### 4. "All Entries" Panel

**UI** (`index.html` + `css/styles.css`):
- Add floating action button (FAB) on elicitation screen: "All Entries" icon
- Slide-in panel overlay with:
  - Search input (filter by reference or gloss)
  - Scrollable list showing: Reference, Gloss, completion badge (✓ if completed)
  - Click entry → jump to that index, close panel

**Logic** (`js/app.js`):
- Add `showAllEntriesPanel()`, `closeAllEntriesPanel()`, `jumpToEntry(index)`
- Filter entries on search input
- Update current index and call `displayCurrentEntry()`

#### 5. Resume Last Position

**Storage** (`js/app.js`):
- Save `localStorage.setItem('lastEntryIndex', this.currentEntryIndex)` on navigation
- Load on elicitation screen init:
  ```javascript
  const saved = localStorage.getItem('lastEntryIndex');
  if (saved !== null) this.currentEntryIndex = Math.min(parseInt(saved), this.entries.length - 1);
  ```

#### 6. Service Worker Update Detection

**SW** (`service-worker.js`):
- Increment cache name on changes: `wordlist-elicitation-v2`, etc.
- On `activate`, postMessage to clients: `{ type: 'SW_UPDATED' }`
...
- Listen for SW messages:
  ...
  navigator.serviceWorker.addEventListener('message', event => {
    if (event.data?.type === 'SW_UPDATED') {
      showUpdateToast(); // "Update available — Reload?"
    }
  });
  ```
- On user confirmation, `postMessage({ type: 'SKIP_WAITING' })` to SW and `location.reload()`

**UI**: Toast/snackbar at bottom with action button

#### 7. Online Import UI

**Import Screen** (`index.html`):
- Two buttons: "Import from File" and "Import from URL"
- URL input: text field with "https://" placeholder
- Validate URL scheme and show loading state

**Logic** (`js/app.js`):
- Add `async handleUrlImport(url)`:
  - Fetch XML with timeout
  - Pass to `parseWordlistFromSource(url, true)`
  - Same flow as file import (clear, add, reload)

#### 8. Enhanced Progress Tracking

**Metrics** (`js/storage.js`):
- Add `getWithAudioCount()`, `getWithTranscriptionCount()`

**Home Screen** (`index.html`):
- Show 4 stats: Total, Completed, With Audio, Transcribed
- Progress bar visualization (optional)

#### 9. Audio Filename Enforcement

**Utility** (`js/utils.js` - new file):
```javascript
function slugifyGloss(gloss) {
  return gloss
    .toLowerCase()
    .replace(/\s+/g, '.')
    .replace(/[^a-z0-9._-]/g, '')
...
function generateAudioFilename(reference, gloss) {
  const slug = slugifyGloss(gloss);
  return `${reference.padStart(4, '0')}_${slug}.wav`;
}
```

Use in `app.js` when saving audio

#### 10. Development Server

**`server.js`** (at repo root):
```javascript
...
http.createServer((req, res) => {
    ...
    if (err) {
      ...
      fs.readFile(filePath, (err2, data2) => {
        if (err2) {
          ...
          res.writeHead(200, { 'Content-Type': 'text/html', 'Service-Worker-Allowed': '/' });
          res.end(data2);
        }
      });
    } else {
  ...
  console.log(`PWA dev server running at http://127.0.0.1:${PORT}/`);
});
```
...
- Unsupported encoding → attempt UTF-8 fallback, warn user
...
- WAV conversion failure → fall back to original format, log warning
...
**Offline**:
- Service worker caching → all assets available offline
- IndexedDB persistence → survives page refresh
- Update detection → notify user, don't break existing functionality

### Success Criteria Checklist

- ✅ 16-bit WAV capability detected; unsupported browsers blocked
- ✅ Import handles UTF-8/16LE/16BE with BOM detection
- ✅ Export produces UTF-16LE XML with BOM
...
- ✅ Update toast shown when new SW version activates
- ✅ Online import via URL works with CORS
- ✅ Progress shows: Total, Completed, With Audio, Transcribed
- ✅ Works completely offline after first load
- ✅ Dev server with proper MIME types and SPA fallback
